...
sdo_hh_proj <- tbl(
  ...
  select(area_code, year, age_group_id, age_group = age_group_description, hh = total_households)
...
sdo_pop <- tbl(
  ...
  mutate(
    ...
    summarize(pop=sum(totalpopulation), .by = c(area_code, year, age_group_id))
...
``` {r}
acs_hr_totl_persons_sdo <- acs_hr_srvy |>
  survey_count(YEAR, age_group_id, name = "pop_acs")

acs_hr_hh_heads_sdo <- acs_hr_srvy |>
  filter(PERNUM == 1) |>
  survey_count(YEAR, age_group_id, name = "hh_acs")

acs_hr_sdo <- acs_hr_totl_persons_sdo |> 
  left_join(acs_hr_hh_heads_sdo) |> 
  filter(!is.na(age_group_id)) |> 
  mutate(headship_rate_acs = hh_acs/pop_acs) |> 
  rename(year = YEAR)

acs_sdo_comparison <- acs_hr_sdo |> 
  full_join(sdo_combined) |> 
