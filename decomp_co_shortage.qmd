...
hh_df <- read_ipums_micro("./data/ipums_raw/usa_00063.xml") %>%
  ...
  as_duckdb_tibble() |>
    ...
    filter(GQ %in% c(0,1,2,5) & YEAR == 2000) %>%
...
hh_df <- bind_rows(hh_df, hh_99)
```

```{r}
hh_df <- as_duckdb_tibble(hh_df |> 
  ))
...
CO_UNDERPOD_HH_DECOMP <- (sum(target_hh_df$MISSINGHH) + CO_HOUSEHOLDS) /
  (1 - VACANCY_RATE) -
  (CO_HOUSING_UNITS - CO_SEASONAL_HOUSING - CO_UNIHABITABLE_HOUSING)
```
