...
CO_UNIHABITABLE_HOUSING <- housing_df |>
  filter(UNIHABITABLE) |>
  count(YEAR, wt = HHWT) |>
  bind_rows(tibble(YEAR = c(2000:2019), n = rep(0, 20))) |> #add in years with no data to allow for analysis later
  arrange(YEAR) |>
  pull(n, name = "YEAR")
...
simple_target_hh_df <- hh_df |>
  filter(STATEFIP == 8, AGE >= 15, YEAR == baseline_year) |>
  group_by(AGEGROUP) |>
  summarise(NEWHH = weighted.mean(HEADSHIP, PERWT), .groups = "drop") |>
  right_join(
    hh_df |>
      filter(STATEFIP == 8, AGE >= 15, YEAR == analysis_year),
    by = "AGEGROUP"
  ) |>
  group_by(AGEGROUP) |>
  summarise(
...
``` {r}
# convert prior analysis to function for passing multiple baseline and/or analysis years
calc_decomp <- function(analysis_year = 2023, baseline_year = 2000) {
  superset_df <- hh_df |>
    filter(YEAR %in% c(analysis_year, baseline_year)) |>
    filter(AGE >= minimum_age & AGE <= maximum_age) |>
    filter(STATEFIP == 8) |>
  ...
  target_hh_df <- hh_df |>
    ...
    relocate(baseline_year, analysis_year, .before = 1)

  CO_MISSING_HH_DECOMP <- tibble(
    baseline_year = baseline_year,
    analysis_year = analysis_year,
    target_hh = sum(target_hh_df$NEWHH),
    current_hh = sum(target_hh_df$HH),
    missing_hh = sum(target_hh_df$MISSINGHH),
    households = CO_HOUSEHOLDS[[as.character(analysis_year)]],
    vacancy_rate = VACANCY_RATE,
    housing_units = CO_HOUSING_UNITS[[as.character(analysis_year)]],
    seasonal_housing = CO_SEASONAL_HOUSING[[as.character(analysis_year)]],
    uninhabitable_housing = CO_UNIHABITABLE_HOUSING[[as.character(
      analysis_year
    )]],
    underproduction = (missing_hh + households) /
      (1 - vacancy_rate) -
      (CO_HOUSING_UNITS[[as.character(analysis_year)]] -
        CO_SEASONAL_HOUSING[[as.character(analysis_year)]] -
        CO_UNIHABITABLE_HOUSING[[as.character(analysis_year)]])
  ...
  outputs <- list(
    missing_hh = CO_MISSING_HH_DECOMP,
    by_age = target_hh_df
  )
...
``` {r}
# run for all analysis years
co_missing_decomp_all <- map(
  c(2000:2023),
  ~ calc_decomp(analysis_year = ., baseline_year = 2000)
) |>
  set_names(c(2000:2023))

calc_decomp(analysis_year = 2000, baseline_year = 2000)
```

```{r}
co_missing_df <- co_missing_decomp_all |>
  bind_rows()

write_csv(
  co_missing_df$missing_hh |>
    distinct() |>
    pivot_longer(
      cols = c(
        target_hh,
